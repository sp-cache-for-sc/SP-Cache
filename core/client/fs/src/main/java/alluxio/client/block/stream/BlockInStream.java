...
public class BlockInStream extends InputStream implements BoundedStream, Seekable,
  ...
  private final String mLog; // for worker traffic loads
  private static ConcurrentHashMap<String, Long> mLoads = new ConcurrentHashMap<String, Long>(); // to record the total traffic load of each worker.
  ...
  protected BlockInStream(PacketReader.Factory packetReaderFactory, BlockInStreamSource blockSource,
    ...
    mLog = System.getProperty("user.dir") + "/logs/workerLoads.txt"; //log the traffic load of workers
  }
  ...
  public int read(byte[] b, int off, int len) throws IOException {
    ...
    try {
      //LOG.info("yinghao debug:"+mPacketReader.getClass());
      if (mPacketReader instanceof LocalFilePacketReader) {
        //LOG.info("yinghao debug: read from local (bytes):" + toRead);
      } else if (mPacketReader instanceof NettyPacketReader) {
        //LOG.info("yinghao debug: read from remote (bytes):" + toRead);
        String host = ((NettyPacketReader) mPacketReader).getWorkerHostName();
        //LOG.info("yinghao debug: hostname:" + host);
        //mLoads.putIfAbsent(host, 0L); // atomic operation
        //mLoads.put(host, mLoads.get(host)+toRead);
        fw = new FileWriter(mLog, true); //the true will append the new data
        fw.write(String.format("" + host + "\t" + toRead + "\n"));
        fw.close();
      }
    }
    catch(Throwable e ) {
      //throwable = e;
      //LOG.info("yinghao debug: error caught:");
      e.printStackTrace();
      if(fw != null)
        fw.close();
      close(); //may fail to close stream on exception
    }
    /**
     finally {
     if( throwable != null) {
