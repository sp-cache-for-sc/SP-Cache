...
public final class RetryHandlingFileSystemMasterClient extends AbstractMasterClient
  ...
  @Override
  public synchronized void spRepartition(final boolean isParallel) throws IOException{
    retryRPC(new RpcCallable<Void>() {
      @Override
      public Void call() throws TException {
        mClient.spRepartition(new RepartitionTOptions().setParallel(isParallel));
        System.out.println("Repartition in Retry fs master client Completes.");
        return null;
      }
    });
  }
  ...
  @Override
  public synchronized void getK() throws IOException{
    retryRPC(new RpcCallable<Void>() {
      @Override
      public Void call() throws TException {
        mClient.getK(new GetKTOptions());
        return null;
      }
    });
  }
