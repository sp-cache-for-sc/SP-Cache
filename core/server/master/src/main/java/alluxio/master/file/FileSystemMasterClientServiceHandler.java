...
public final class FileSystemMasterClientServiceHandler implements
  ...
  @Override
  public RepartitionTResponse spRepartition(final RepartitionTOptions options) throws AlluxioTException{
    return RpcUtils.callAndLog(LOG, new RpcCallableThrowsIOException<RepartitionTResponse>() {
      @Override
      public RepartitionTResponse call() throws AlluxioException, IOException {
        mFileSystemMaster.spRepartition(options.isParallel());
        return new RepartitionTResponse();
      }
    });
  }

  @Override
  public GetKTResponse getK(final GetKTOptions options) throws AlluxioTException{
    return RpcUtils.callAndLog(LOG, new RpcCallableThrowsIOException<GetKTResponse>() {
      @Override
      public GetKTResponse call() throws AlluxioException, IOException {
        mFileSystemMaster.getK();
        return new GetKTResponse();
      }
    });
  }
